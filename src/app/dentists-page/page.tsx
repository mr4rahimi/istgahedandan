    ...
    <div style={{ direction: "rtl", fontFamily: "inherit", background: "#f4f9fb", color: "#16313b", minHeight: "100vh", paddingBottom: 80 }}>
      ...
      <section style={{ maxWidth: 1280, margin: "0 auto", padding: "20px 16px 6px" }}>
        <div className="ih-hide-scroll" style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 6 }}>
          <Link href="/dentists-list" style={{ border: !locationFilter ? "none" : "1px solid #d2e8ee", background: !locationFilter ? "linear-gradient(135deg,#15b8d1,#0a6f9e)" : "#fff", color: !locationFilter ? "#fff" : "#2a4f5b", textDecoration: "none", fontWeight: 600, fontSize: 14, padding: "9px 18px", borderRadius: 30, whiteSpace: "nowrap", flexShrink: 0 }}>همه مناطق</Link>
          {locations.map(loc => (
            <Link key={loc.slug} href={`/dentists-list?location=${loc.slug}`} style={{ border: locationFilter === loc.slug ? "none" : "1px solid #d2e8ee", background: locationFilter === loc.slug ? "linear-gradient(135deg,#15b8d1,#0a6f9e)" : "#fff", color: locationFilter === loc.slug ? "#fff" : "#2a4f5b", textDecoration: "none", fontWeight: 600, fontSize: 14, padding: "9px 18px", borderRadius: 30, whiteSpace: "nowrap", flexShrink: 0 }}>{loc.title}</Link>
          ))}
        </div>
      </section>
      ...
      {totalPages > 1 && (
        <section style={{ maxWidth: 1280, margin: "0 auto", padding: "16px 16px 46px" }}>
          <div style={{ display: "flex", justifyContent: "center", gap: 8, flexWrap: "wrap" }}>
            {page > 1 && (
              <Link href={`/dentists-list?page=${page - 1}${locationFilter ? `&location=${locationFilter}` : ""}`} style={{ width: 42, height: 42, border: "1px solid #d7e6ea", background: "#fff", borderRadius: 12, display: "grid", placeItems: "center", textDecoration: "none", color: "#2a4f5b" }}>
            ...
            {Array.from({ length: Math.min(5, totalPages) }, (_, idx) => {
              ...
              return (
                <Link key={p} href={`/dentists-list?page=${p}${locationFilter ? `&location=${locationFilter}` : ""}`} style={{ width: 42, height: 42, border: p === page ? "none" : "1px solid #d7e6ea", background: p === page ? "linear-gradient(135deg,#15b8d1,#0a6f9e)" : "#fff", color: p === page ? "#fff" : "#2a4f5b", borderRadius: 12, display: "grid", placeItems: "center", textDecoration: "none", fontWeight: 700 }}>
                  {p}
                </Link>
              );
            })}

...
export const metadata: Metadata = {
  ...
  description: "مجموعه کامل خدمات تخصصی دندانپزشکی از جمله ایمپلنت، لمینت، ارتودنسی و بیشتر.",
};

const SERVICE_GRADIENTS: Record<string, string> = {
  default: "linear-gradient(135deg,#0c5e7c,#0a3f54)",
};
...
const TOOTH_ICON = `<path d="M12 2c-1.5 0-2.7 1-3.3 2.4C8 6 7.5 8 7.5 10.5c0 3 .8 7 2 9 .5.8 1.3 1.2 2 .6.4-.4.6-1.3.7-2.4.2-1.6.3-2.7 1.8-2.7s1.6 1.1 1.8 2.7c.1 1.1.3 2 .7 2.4.7.6 1.5.2 2-.6 1.2-2 2-6 2-9 0-2.5-.5-4.5-1.2-6.1C14.7 3 13.5 2 12 2z"/>`;
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(290px, 1fr))", gap: 22 }}>
          {services.map((svc, i) => (
            <div key={svc.slug} style={{ background: "#fff", border: "1px solid #e7f0f3", borderRadius: 22, overflow: "hidden", boxShadow: "0 16px 36px -28px rgba(13,75,107,.5)", display: "flex", flexDirection: "column" }}>
              <div style={{ height: 168, position: "relative", background: gradients[i % gradients.length] }}>
                ...
                {svc.featuredImage && (
                  <img src={svc.featuredImage} alt={svc.title} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />
                )}
                <span style={{ position: "absolute", bottom: -26, right: 20, width: 56, height: 56, borderRadius: 16, background: "#fff", boxShadow: "0 8px 18px -8px rgba(13,75,107,.4)", display: "grid", placeItems: "center" }}>
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#0c8aa6" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" dangerouslySetInnerHTML={{ __html: TOOTH_ICON }} />
                </span>
              </div>
              <div style={{ padding: "34px 20px 20px", display: "flex", flexDirection: "column", flex: 1 }}>
                ...
                <p style={{ margin: "0 0 18px", fontSize: 14, lineHeight: 1.95, color: "#6c8b95", flex: 1 }}>{svc.shortDesc || "ارائه خدمات تخصصی دندانپزشکی با بهترین کیفیت"}</p>
